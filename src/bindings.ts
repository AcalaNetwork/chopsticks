...
export const setupBindings = (chain: Blockchain) => {
  global.getStorage = async function (blockHash: HexString, key: HexString) {
  ...
  global.getPrefixKeys = async function (blockHash: HexString, key: HexString) {
    const block = await chain.getBlock(blockHash)
    if (!block) throw Error(`Block not found ${blockHash}`)
    return block.getKeysPaged({ prefix: key, pageSize: 1000, startKey: key })
  }

  global.getNextKey = async function (blockHash: HexString, key: HexString) {
    const block = await chain.getBlock(blockHash)
    if (!block) throw Error(`Block not found ${blockHash}`)
