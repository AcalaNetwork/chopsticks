...
use crate::light_client::{monotonic_clock_us, JsLightClientCallback};

pub(crate) fn timer_finished(js_callback: Arc<JsLightClientCallback>) {
    process_timers(js_callback);
}
...
impl Delay {
    pub fn new(after: Duration, js_callback: Arc<JsLightClientCallback>) -> Self {
        let now = Duration::from_micros(monotonic_clock_us());
        Self::new_inner(now + after, js_callback)
    }

    pub fn new_at_monotonic_clock(when: Duration, js_callback: Arc<JsLightClientCallback>) -> Self {
        Self::new_inner(when, js_callback)
    }

    fn new_inner(when: Duration, js_callback: Arc<JsLightClientCallback>) -> Self {
        let now = Duration::from_micros(monotonic_clock_us());
        ...
        if lock
            ...
            == timer_id
        {
            js_callback.start_timer((when - now).as_millis() as u64);
        }
...
fn process_timers(js_callback: Arc<JsLightClientCallback>) {
    // Because we're in a single-threaded environment, `try_lock()` should always succeed.
    let mut lock = TIMERS.try_lock().unwrap();
    let lock = &mut *lock;
    let now = Duration::from_micros(monotonic_clock_us());
    ...
    if let Some(next_wakeup) = next_wakeup {
        ...
        js_callback.start_timer(duration.as_millis() as u64);
    } else {
