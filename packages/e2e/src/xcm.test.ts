...
describe('XCM', async () => {
  const withNetwork = async (network: () => Promise<Network>, test: (network: Network) => Promise<void>) => {
  ...
  const withXcmNetworks = async (test: (context: { acala: Network; polkadot: Network }) => Promise<void>) => {
    let acala: Network | undefined
    let polkadot: Network | undefined

    try {
      acala = await networks.acala()
      polkadot = await networks.polkadot()
      await test({ acala, polkadot })
    } finally {
      try {
  ...
  it('Acala handles downward messages', async () => {
    await withNetwork(networks.acala, async (acala) => {
  ...
  it('Polkadot send downward messages to Acala', async () => {
    await withXcmNetworks(async ({ acala, polkadot }) => {
